  ...
  <>
      ...
      <div className="shell grid gap-12 lg:grid-cols-2 lg:items-center">
        <div className="relative overflow-hidden rounded-[2rem] bg-ink p-6 shadow-2xl sm:p-9">
          ...
          <div className="relative">
            <div className="mb-8 flex flex-wrap items-center justify-between gap-3"><span className="rounded-full bg-mint/12 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-mint">Live internal case study</span><span className="text-xs text-slate-copy">Asili Eco-Wellness</span></div>
            <div className="rounded-[1.5rem] bg-[#f7f3e9] p-5 text-[#213326] sm:p-7">
              <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[#dfe6d7] pb-5"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#74856f]">Batch passport</p><p className="mt-1 font-display text-xl font-bold">Makueni honey</p></div><span className="rounded-lg bg-[#213326] px-3 py-2 text-[10px] font-bold text-white">Verified record</span></div>
              <div className="mt-6 grid grid-cols-1 gap-3 min-[420px]:grid-cols-2"><div className="min-w-0 rounded-xl bg-white p-4"><p className="text-[10px] text-[#74856f]">Origin</p><p className="mt-1 text-sm font-bold">Makueni, Kenya</p></div><div className="min-w-0 rounded-xl bg-white p-4"><p className="text-[10px] text-[#74856f]">Batch</p><p className="mt-1 break-words text-sm font-bold">SAMPLE-2604-01</p></div></div>
        ...
        <div><span className="eyebrow">Proof through practice</span><h2 className="section-title mt-5 !text-ink">Turning a product website into a clearer trust and traceability experience.</h2><p className="body-lg mt-6 text-slate-600">For Asili, we refined the customer proposition, improved the digital journey, and developed a QR-linked batch passport demonstration that organizes origin and product information.</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3"><Link to="/portfolio/asili-business-helper" className="btn-dark">Read case study <ArrowUpRight className="h-4 w-4" /></Link><a href="https://asilii.netlify.app" target="_blank" rel="noreferrer" className="btn-light !border-slate-200 !bg-white !text-ink hover:!bg-cloud">Visit Asili <ExternalLink className="h-4 w-4" /></a><a href="https://asilii.netlify.app/b/sample-2604-01" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-3 text-sm font-bold text-cobalt">View batch passport <ArrowUpRight className="h-4 w-4" /></a></div>
        </div>
      </div>
    </section>
    ...
    <section id="about" className="bg-white py-24 text-ink sm:py-32">
      <div className="shell grid gap-12 lg:grid-cols-[.88fr_1.12fr] lg:items-center">
        <div className="rounded-[2rem] bg-ink p-6 sm:p-10"><p className="eyebrow-light">Founder-led consultancy</p><div className="mt-10 flex min-w-0 items-center gap-4 sm:gap-5"><div className="logo-mark grid h-14 w-14 shrink-0 place-items-center rounded-2xl font-display text-xl font-bold text-white sm:h-16 sm:w-16">KS</div><div className="min-w-0"><h3 className="text-2xl font-bold">Kevin Sila</h3><p className="mt-1 text-sm text-slate-copy">Founder · Business systems & operational intelligence</p></div></div><p className="mt-8 text-sm leading-7 text-slate-copy">Based in Kenya and equipped for remote delivery, Silatech combines business operations, data systems, research coordination, digital platforms, dashboards, and practical AI workflows.</p><a href={SOCIAL_LINKS.portfolio} target="_blank" rel="noreferrer" className="btn-light mt-8 w-full sm:w-auto">View Kevin’s portfolio <ArrowUpRight className="h-4 w-4" /></a></div>
